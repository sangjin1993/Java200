 ...
 * * 생성자는 new예약어로 객체를 생성할 때 한 번 호출되는 초기화용 메서드이다. 생성자는 public String(char[])처럼 "제한자 클래스이름(인자)"의 형태로 사용된다.
 * * 메서드는 public int charAt(index)와 같이 "제한자 리턴타입 메서드이름(인자)의 형태를 튀한다.
 * * "제한자(modifier)"는 int타입이므로 문자열로 변환해야 보기 편리하다. 예를 들어 1은 "public", 2는 "private"이다. 아규먼트나 리턴타입에서 배열은 "["로
 *   시작하고, 타입은 "[C"처럼 "["뒤에 대문자를 붙인다. "[B"는 byte, "[C"는 char[]이다.
 * 
 */
